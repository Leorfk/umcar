<%-- 
    Document   : EditarAutomovel
    Created on : 10/04/2019, 16:53:01
    Author     : Kirkh
--%>
...
<html>
    <head>
        ...
        <div>    
            ...
            <div>    
                <div>
                    <h2 class="text-center">Editar Automóvel</h2>
                </div>
                <form action="ControleAutomovel" method="POST" role="form" data-toggle="validator" class="container">           
                    <% Automovel automovel = new Automovel();
                       automovel = (Automovel)request.getAttribute("automovel");
                    %>  
                    <input type="hidden" name="idAutomovel" value="<%=automovel.getIdAutomovel()%>"/>   
                    
                        <div class="row">
                            <div class="form-group col-md-2">
                                <label for="txtPlaca" class="control-label">Número da Placa</label>
                                <input type="number" name="txtPlaca" required="" class="form-control" value="<%=automovel.getPlaca() %>">
                            </div>
                            <div class="form-group col-md-4">
                                <label for="txtModelo" class="control-label">Modelo</label>
                                <input type="text" name="txtModelo" required="" class="form-control" value="<%=automovel.getModelo() %>">
                            </div>
                            <div class="form-group col-md-4">
                                <label for="txtMarca" class="control-label">Marca</label>
                                <input type="text" name="txtMarca" required="" class="form-control" value="<%=automovel.getMarca()%>">    
                            </div>                   
                        </div>
                        <div class="row">
                            <div class="form-group col-md-4">
                                <label for="txtTipo" class="control-label">Tipo do Automóvel</label>
                                <input type="text" name="txtTipo" required="" class="form-control" value="<%=automovel.getTipoAutomovel()%>">  
                            </div>
                            <div class="form-group col-md-3">
                                <label for="txtPortas" class="control-label">Portas</label>
                                <select name="txtPortas" required="" class="form-control" >
                                    <option selected value="<%=automovel.getPortas()%>"><%=automovel.getPortas()%></option>
                            ...
                            <div class="form-group col-md-3">
                                ...
                                <select name="txtPassageiros" required="" class="form-control">
                                    <option selected value="<%=automovel.getPassageiros()%>"><%=automovel.getPassageiros()%></option>                              
                       ...
                       <div class="row"> 
                            <div class="form-group col-md-4">
                                ...
                                <input type="text" name="txtCor" required="" class="form-control" value="<%=automovel.getCor() %>">
                            </div>
                            <div class="form-group col-md-4">
                                <label for="txtChassi" class="control-label">Nº Chassi</label>
                                <input type="text" name="txtChassi" step="any" required="" class="form-control" value="<%=automovel.getChassi()%>">
                            </div>
                            <div class="form-group col-md-2">
                                <label for="txtVenda" class="control-label">Valor para Venda</label>
                                <input type="number" name="txtVenda" step="any" required="" class="form-control" value="<%=automovel.getValorVenda()%>">
                            </div>
                        </div>
                        <div class="row">                            
                            <div class="form-group col-md-5">
                                <label for="txtObservacoes" class="control-label">Observações</label>
                                <input type="text" name="txtObservacoes" step="any" class="form-control" value="<%=automovel.getObservacoes()%>">
                            </div>
                        </div>
                        <div>
                            <button type="submit" value="Atualizar" name="acao" class="btn btn-info">Salvar</button>
